...
%% Double-blind review: author information is in the separate title page.
...
spanning tightening doses from T\"{u}rkiye's 36.0 percentage points to
Vietnam's 1.5. The price of debt proves recoverable: a
simple implied borrowing cost tracks each country's policy path, and its change
tracks tightening dose across the sample---a pattern we read qualitatively
given only five countries---with pass-through of roughly one-third in
T\"{u}rkiye and coverage halving. Exposure does
not. Split-half reliabilities of $+0.045$ and $+0.054$ in the two countries with
decade-long history are indistinguishable from zero, implying an attenuation
factor of essentially zero, T\"{u}rkiye's
...
interaction is null on interest coverage and profitability ($p=0.84$,
$p=0.41$) while significant only on the price measure already shown to move
...
from 9.0\% to 50\%, under inflation exceeding 60\%; Vietnam tightened
...
largest and most reliable firm counts.\footnote{Four further markets were
assembled on the same basis during scoping and are not carried into the
reported five-country test, and we name all four rather than a convenient
subset. Chile (7.25pp dose, 80 firms) sits squarely on the same dose-response
line as the five reported countries; its exclusion is for parsimony alone.
Brazil (5.75pp dose, 215 firms) is the one clear outlier in the full assembled
set, with a change in implied borrowing cost of only 0.48 percentage points
against a pass-through ratio roughly a quarter of every other country's---
0.48/5.75 against a range of 0.16 to 0.62 elsewhere. We report this so the
reader can judge for themselves: the five-country correlation below is not
conditioned on having found and discarded an inconvenient point after the
fact, but its absence should be read as a reduction in scope, not as evidence
that Brazil disproves the pattern. Hungary (24 firms) and Mexico (19 firms)
were excluded for the reason given in the text: samples thinner than the
83-firm Turkish sample already shown, in Section~\ref{sec:robust}, to produce
a false positive before the sample expanded. A sixth market, Thailand, was
attempted on the same basis and yielded no usable data at all---every ticker
returned an empty statement, evidently a data-vendor outage rather than a
substantive gap---and is recorded here for completeness rather than omitted
from the account of what was tried.}
Across the five countries the change in
implied borrowing cost correlates with tightening dose at $r=0.995$
...
Two results follow. We quantify pass-through: a 41.0 percentage point Turkish
policy move---from the 9.0\% low point at end-2022 to the true 2024 peak of
50\%, sustained March through November before a December cut to
47.5\%---produces a 13.4 point move in realised borrowing costs, roughly a
third, while median interest coverage halves from 3.36 to 1.66.
...
$+0.045$ in T\"{u}rkiye and $+0.054$ in Vietnam, implying
...
under the null, $k/(T-1)\approx0.50$, against observed medians of $0.66$ in
Vietnam and $0.71$ in T\"{u}rkiye---not an exact match, but the same order of
magnitude as chance, which is the point: a two-parameter fit on five annual
...
survives Proposition~\ref{prop:flow}. \citet{jeenas2019} shows that firms with
less liquid balance sheets cut investment more sharply after a monetary
contraction because they are more exposed to fluctuations in the cost of new
borrowing---the same repricing exposure this paper tries, and fails, to recover
from firm-level accounting data.
...
currency, and \citet{eichengreenhausmannpanizza2003,eichengreenhausmannpanizza2005}
subsequently distinguish it from the related but distinct problem of debt
intolerance and trace its consequences for output and capital-flow volatility.
\citet{salomaovarela2022} model the firm-level counterpart directly, showing
that foreign-currency borrowing arises from a dynamic trade-off between
currency risk and growth rather than a uniform exposure, which is itself a
reason to expect the cross-firm heterogeneity our design tries and fails to
recover. The empirical record is more nuanced than the original framing
suggests: \citet{bleakley2008} find that firms holding dollar debt did not
systematically underperform during depreciations, because they tended to hold
dollar revenues too, while \citet{aguiar2005} documents contractionary
balance-sheet effects in Mexico. \citet{kalemli2016} reconcile these by
showing that the damaging combination is unhedged FX debt together with a
banking crisis restricting credit supply.
...
validity test as obligatory rather than optional. \citet{hausman2001} surveys
the broader consequences of mismeasured variables on both sides of a
regression, including why the standard instrumental-variables fix for
attenuation is unavailable precisely when the instrument is itself only weakly
correlated with the true regressor---the situation our split-half correlation
diagnoses directly, so we do not pursue an errors-in-variables correction:
instrumenting with one half of the estimation window for the other would
require exactly the reliability the data reject. \citet{boundbrownmathiowetz2001}
review the wider empirical record on measurement error in survey and
administrative data and note that classical assumptions---error uncorrelated
with the true value and independent across variables---are themselves
frequently violated in ways that can push bias in either direction, which is
part of why we treat the split-half correlation as a direct test rather than
inferring reliability from auxiliary assumptions about the error structure.
...
values of $+0.045$ and $+0.054$, with 95\% confidence intervals of
$[-0.074,\,0.164]$ in T\"{u}rkiye and $[-0.033,\,0.142]$ in Vietnam (Fisher
$z$-transform, computed from 200 independent random half/half partitions of
each firm's usable years and the median number of firms entering a partition).

\begin{remark}[Corollary~\ref{cor:uninformative} as a test]
\label{rem:test}
Corollary~\ref{cor:uninformative}'s condition is testable rather than merely
descriptive. Both confidence intervals contain zero, so the null
$H_0:\lambda=0$ cannot be rejected in either country; more decisively, both
intervals' upper bounds sit far below any conventional reliability threshold
(commonly $0.70$--$0.80$ in psychometric practice), so $H_0:\lambda\ge0.5$ is
rejected at the 5\% level in both countries. The design does not merely fail
to demonstrate reliability---it affirmatively rejects it at any level that
would license treating $\widehat{\theta}$ as informative.
\end{remark}
...
data. When we do so in Section~\ref{sec:exposure} the condition looks
satisfied: sampling noise implies $\lambda\approx0.923$ in Vietnam, comfortably
above conventional reliability thresholds, and $0.664$ in T\"{u}rkiye, at or
above the more permissive end of them---so a conventional power calculation
would have judged the design feasible, or close to it, at the available panel
length.

That verdict is contradicted by the split-half correlations, which are close to
zero (Section~\ref{sec:exposure} reports $+0.054$ and $+0.045$). The
contradiction is informative, and resolving it requires enriching
\eqref{eq:proxy}.
...
decomposition in Section~\ref{sec:exposure} attributes 94.6\% of Vietnamese and
95.5\% of Turkish dispersion in estimated slopes to the transitory component and
only a small remainder to the permanent one---so the binding problem is
impermanence, and quarterly estimation would not solve it.
...
With $k=2$ and a median of $T=5$ usable observations on the split-half windows
this benchmark defines, $\E[R^2]=0.50$. Any reported fit near this value is
uninformative. Section~\ref{sec:exposure} reports observed medians of $0.66$ in
Vietnam and $0.71$ in T\"{u}rkiye.
...
\begin{remark}[A limit of the invariance argument]
\label{rem:monetarygainloss}
Proposition~\ref{prop:flow} assumes every flow is restated by the same
firm-invariant factor $\psi_t$. IAS~29/TMS~29 restatement introduces one
further line that does not fit this template: a net monetary gain or loss,
recognised in income and equal to the erosion or accretion of purchasing
power on the firm's net monetary position (monetary assets less monetary
liabilities) over the period. This gain or loss is a function of each firm's
own net monetary position, not a common multiplicative factor, so a flow
measure that includes it---as EBIT can, depending on where the vendor's
statement places the line---is not covered by the proof above. We do not
observe the net monetary position separately in our data and cannot rule out
some residual contamination of the Turkish EBIT margin on this channel; the
implied borrowing cost is unaffected, since interest expense and average debt
are both defined independently of the net monetary position.
\end{remark}
...
at 50.0\% in March 2024 and held there through November, before a December cut
to 47.5\%; it stood at 38.0\% at end-2025. Consumer price inflation
...
2022 amid stress in the domestic corporate bond market---a crisis of fraudulent
issuance and mass defaults that froze new issuance and, \citet{huynhphan2024}
show, tightened the cost and availability of bank debt for listed firms more
broadly, independent of the policy-rate move itself---and reversed through
...
dose-response test of Section~\ref{sec:dose} but not the decade-long
event-study test Section~\ref{sec:mainresults} also requires for T\"{u}rkiye and
Vietnam; and by the feasibility condition of Section~\ref{sec:framework}, a
...
Entry is not the whole story, and we state the remaining threat plainly
rather than let the firm-fixed-effects argument above stand in for it. Both
ticker universes are enumerated from a current listing snapshot: the Turkish
panel is built by fetching statements for every presently BIST-listed company,
and the three-country extension similarly enumerates each exchange's current
constituents. A firm that delisted, was acquired, or failed at any point in
2010--2025 is therefore absent from the panel not only after its exit but in
every year it was ever observed, since it was never fetched in the first
place. This is a materially different problem from the balanced-panel
question fixed effects answer: an unbalanced panel with random entry and exit
is unbiased under within-firm estimation, but non-random attrition---firms
disappearing \emph{because} of the shock we study---is not, and firm fixed
effects do nothing to correct for a firm never entering the sample to begin
with. For a paper whose outcomes are interest coverage, EBIT margin and debt
growth under a severe monetary tightening, the firms most likely to have
failed \emph{because of} that tightening are exactly the ones a
current-snapshot ticker universe is least likely to include. We do not have a
correction for this and flag it as an open limitation: the null we report
describes the population of firms that survived to be listed as of data
collection, and if distress-driven delisting is correlated with exposure, the
true cross-sectional gradient among firms that failed could differ from what
we estimate among firms that did not.
...
\small\textit{Note:} the CBRT column reports the December (year-end) reading
each year, for consistency across the multi-year path. The true within-year
peak was 50.0\%, sustained from March through November 2024, before a
December cut to the 47.5\% shown; Section~\ref{sec:price} uses the true peak,
not the year-end reading, wherever pass-through is quantified.
\end{minipage}
\end{table}

Figure~\ref{fig:validation} plots the implied borrowing cost against each
country's policy path directly.
...
Vietnamese sample minimum of 1.96 in exactly 2023.

\subsubsection{Pass-through}

Policy moved 41.0 points between end-2022 (9.0\%) and the true 2024 peak
(50.0\%, sustained March through November, not the 47.5\% year-end reading
after the subsequent December cut); realised borrowing costs moved 13.4
points, from 17.8\% to 31.2\%. By Corollary~\ref{cor:passthrough} this
identifies a mean effective repricing share of $\bar{m}\approx0.33$. Set
against a median short-term debt share of $0.63$, it implies that even
nominally short debt repriced only partially. Over the same window median
interest coverage halved, from 3.36 to 1.66. Vietnam's mild cycle produces a
proportionally mild response: costs move 0.76 points from 2022 to the 2023
tightening year, and coverage dips from 2.91 to 1.96 before recovering to
3.19 as policy reverses.
...
On conventional diagnostics the estimates behave: 71\% of Vietnamese firms and
...
of funds by roughly 0.79 points, and median in-sample $R^2$ is 0.29 in Vietnam
and 0.34 in T\"{u}rkiye on the full 2012--2022 estimation window---a distinct
quantity from the split-half-window fit of 0.66 and 0.71 reported above and
in Table~\ref{tab:split} below, since the two are computed on windows of
different length. Propositions~\ref{prop:split}--\ref{prop:r2} supply the
...
\label{tab:split}
\noindent\footnotesize Each firm's usable years are partitioned into two
halves 200 times, independently at random; the table reports the
correlation between the two halves' estimates, averaged across the 200
partitions, by country. The computation is scripted in full in the
accompanying replication code.\par\vspace{0.4em}
\begin{tabular}{lrr}
\toprule
Measure & T\"{u}rkiye & Vietnam \\
\midrule
Repricing sensitivity $\widehat{\theta}$ & $\mathbf{+0.045}$ & $\mathbf{+0.054}$ \\
FX sensitivity $\widehat{\gamma}$        & $+0.003$ & $+0.059$ \\
Short-term debt share                    & $+0.828$ & $+0.903$ \\
Export share                             & $+0.935$ & --- \\
\bottomrule
\end{tabular}
\end{table}
...
ratio $\lambda$. Both are within 0.06 of zero, so by
Corollary~\ref{cor:uninformative} the treatment estimand is attenuated to
essentially zero and carries almost no information about $\beta$. The measured
balance-sheet characteristics in the lower rows behave as far more stable
traits, so the failure is specific to regression-estimated quantities.
Table~\ref{tab:decomp} reports the resulting variance decomposition.
...
& Vietnam & T\"{u}rkiye \\
\midrule
$\Var(\widehat{\theta})$                       & $5.25\times10^{-4}$ & $1.30\times10^{-4}$ \\
Sampling component $\sigma^2_u$                & $4.03\times10^{-5}$ & $4.35\times10^{-5}$ \\
Reliability implied by sampling noise alone    & 0.923 & 0.664 \\
Measured split-half correlation                & $+0.054$ & $+0.045$ \\
Permanent component $\sigma^2_\theta$          & $2.85\times10^{-5}$ & $5.88\times10^{-6}$ \\
Transitory component $\sigma^2_\nu$            & $4.96\times10^{-4}$ & $1.24\times10^{-4}$ \\
Transitory share of dispersion                 & \textbf{94.6\%} & \textbf{95.5\%} \\
\bottomrule
\end{tabular}
\end{table}
...
reliabilities of 0.923 and 0.664: a conventional power calculation, or an
inspection of standard errors, would have judged the Vietnamese design
comfortably feasible and the Turkish one feasible or close to it. The
split-half correlations nonetheless leave almost none of that
dispersion permanent, attributing 94.6\% of Vietnamese and 95.5\% of Turkish
dispersion to a transitory component.
...
realised cost. Median in-sample fit on the split-half windows is 0.66 in
Vietnam and 0.71 in T\"{u}rkiye against the null benchmark $k/(T-1)=0.50$ of
...
Figure~\ref{fig:events} plots these coefficients across the full event-study
window.
...
five-country, 2021--2025 sample described in Section~\ref{sec:dataonly}, spanning
tightening doses from T\"{u}rkiye's 36.0 percentage points to Vietnam's 1.5.
Table~\ref{tab:dose} reports the resulting cross-country comparison.
...
\midrule
T\"{u}rkiye & 36.00 & 559 & 10.59 & 3.36 & 2.45 \\
Poland      &  5.00 & 521 &  1.59 & 4.72 & 2.91 \\
Israel      &  4.65 & 274 &  1.90 & 4.81 & 3.29 \\
South Korea &  2.50 & 290 &  1.56 & 6.19 & 4.16 \\
Vietnam     &  1.51 & 570 &  0.13 & 2.91 & 2.23 \\
\bottomrule
\end{tabular}
\end{table}

The change in implied borrowing cost correlates with policy dose at
$r=0.995$ across the five countries, and median interest coverage falls in
every single one, from T\"{u}rkiye's 36.0-point tightening down to Vietnam's
1.5. Figure~\ref{fig:dose} plots the relationship.
...
A correlation of $0.995$ on five points warrants an immediate caveat rather
...
treating $0.995$ as a precisely estimated population parameter. The
...
Section~\ref{sec:mainresults}'s two-country test, $\beta_2$ should be positive and
...
\midrule
Interest coverage  & 598 & $-0.977$ (0.071) & $\phantom{-}0.050$ (0.840) \\
EBIT margin        & 598 & $-0.008$ (0.136) & $-0.009$ (0.414) \\
Implied rate       & 604 & $\phantom{-}0.008$ (0.073) & $\phantom{-}0.032$ (0.047) \\
\bottomrule
\end{tabular}
\end{table}

The dose interaction is null on both distress outcomes: interest coverage
($p=0.840$) and EBIT margin ($p=0.414$) show no evidence that the exposure
gradient steepens with tightening severity. It is significant only for the
implied rate itself ($p=0.047$)---the mechanical restatement, at firm level,
...
Section~\ref{sec:mainresults}'s significant coefficients as genuine pre-existing
trends.
...
in our data they indicated reliabilities of 0.664--0.923 where the split-half
estimate of the true permanent share was under 6\%.
...
escape it, with the qualification of Remark~\ref{rem:monetarygainloss}: a flow
outcome that itself bundles the IAS~29 net monetary gain or loss is only
partially protected.
...
roughly one-third, a halving of median Turkish interest coverage, and a data field
...
proxy, that reliability is identified by a split-half correlation which is
indistinguishable from zero in both countries, and the dispersion in estimated
sensitivities is overwhelmingly transitory rather than permanent---so the
failure is impermanence, which more data cannot cure.

A sceptical reading of this null deserves airing rather than dismissal. The
panel we estimate exposure from is not a random sample of firms subject to
monetary tightening; it is the sample of firms that were listed, filed
statements, and survived long enough to enter a decade-plus estimation window.
If the firms most exposed to repricing risk are disproportionately the ones
that delisted, merged or failed before or during the tightening we study---the
attrition problem flagged as an open limitation in
Section~\ref{sec:dataonly}---then a genuine cross-sectional exposure gradient
could exist in the population of Turkish and Vietnamese firms while being
invisible in the surviving, listed subset we observe. We cannot rule this out
with the data at hand, and we doubt it can be ruled out by any design built
solely on published statements: the same institutional gap that keeps
contractual exposure from being observed also keeps the researcher from
characterising who is missing from the observed sample. This is a second,
independent route to caution about the null, distinct from the measurement-
reliability argument above---not a reason to doubt that argument, but a reason
not to over-read the null as proof the exposure channel is dormant rather than
merely unobservable from where we stand.
...
A result independent of either verdict deserves its own emphasis before
closing. The inflation-accounting confound we formalise
(Proposition~\ref{prop:restate}) and its remedy
(Proposition~\ref{prop:flow}) are not specific to T\"{u}rkiye's tightening or
to floating-rate exposure. Any difference-in-differences design that combines
a treatment date with an accounting regime restating non-monetary items by a
vintage-dependent factor---IAS~29/TMS~29 hyperinflation accounting, but
equally a large one-off revaluation elsewhere---inherits the same
exposure-by-post interaction that two-way fixed effects cannot absorb, and the
same fix: outcomes built from a ratio of two flows are invariant to it, in
logs. We regard this as the paper's most exportable methodological result,
independent of what a reader concludes about the exposure-measurement question
above, and applicable well beyond the two countries and the specific channel
studied here.
