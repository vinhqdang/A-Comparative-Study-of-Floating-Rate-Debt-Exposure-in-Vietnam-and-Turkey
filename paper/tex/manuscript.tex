...
Transmission to Corporate Balance Sheets: A Nine-Country Emerging-Market Test}
...
non-financial firms in nine emerging markets over 2010--2025 (2021--2025 for
seven of the nine), spanning tightening doses from T\"{u}rkiye's 33.5
...
correlates with tightening dose at $r=0.983$ across the sample, with
...
fixed effects cannot absorb, and pooling all nine countries the exposure-by-dose
interaction is null on interest coverage and profitability ($p=0.93$,
$p=0.39$) while significant only on the price measure already shown to move
...
extend the design to seven further emerging markets---Brazil, Chile, Hungary,
Israel, Mexico, Poland and South Korea---covering the full range of monetary
tightening observed globally over 2021--2025, from Hungary's 10.6 percentage
points down to Korea's 2.5. Across all nine countries the change in
implied borrowing cost correlates with tightening dose at $r=0.983$
...
a testable pre-trend, an inflation-accounting stress test; the nine-country
extension supplies the breadth, showing the conclusion is not an artefact of
which two countries were chosen.
...
tests: Vietnam and T\"{u}rkiye, described below. Seven further countries---
Brazil, Chile, Hungary, Israel, Mexico, Poland and South Korea---extend the
2021--2025 window across the range of monetary tightening observed globally in
that cycle, from Hungary's 10.6 percentage points down to Korea's 2.5,
bracketing T\"{u}rkiye above and Vietnam below. These seven are built from
...
retains only four to five annual periods, so these seven support the aggregate
...
Section~\ref{sec:measure} and the exposure test of this section to the full
nine-country, 2021--2025 sample described in Section~\ref{sec:data}, spanning
tightening doses from T\"{u}rkiye's 33.5 percentage points to Vietnam's 0.8.

\begin{table}[htbp]
\centering
\caption{Dose-response across nine countries, 2022 to 2023--24}
\label{tab:dose}
...
Hungary     & 10.60 &  23 &  3.51 & 6.28 & 4.41 \\
Chile       &  7.25 &  80 &  2.24 & 4.15 & 2.75 \\
Brazil      &  5.75 & 215 &  0.48 & 2.48 & 2.05 \\
Mexico      &  5.75 &  19 &  1.94 & 3.62 & 2.59 \\
...
$r=0.983$ across the nine countries, and median interest coverage falls in
every single one, from a 33-fold range of tightening severity down to
Vietnam's near-negligible dose. Figure~\ref{fig:dose} plots the relationship.
This is, to our knowledge, the tightest cross-country validation of a
statement-based transmission measure reported to date, and it extends
Section~\ref{sec:measure}'s two-country result to a design with genuine
cross-sectional variation in treatment intensity rather than two points.
...
dose, nine countries, 2022 to 2023--24. The measure is constructed from firm
statements alone.}
\label{fig:dose}
\end{figure}

We then pool all nine countries and estimate the triple interaction
\begin{equation}
...
\begin{table}[htbp]
\centering
\caption{Pooled exposure $\times$ dose interaction, nine countries}
\label{tab:dosegrad}
\begin{tabular}{lrrrr}
\toprule
Outcome & Firms & $\beta_1$ ($p$) & $\beta_2$ ($p$) \\
\midrule
Interest coverage  & 603 & $-0.979$ (0.069) & $\phantom{-}0.021$ (0.934) \\
EBIT margin        & 603 & $-0.009$ (0.098) & $-0.009$ (0.389) \\
Implied rate       & 610 & $\phantom{-}0.008$ (0.070) & $\phantom{-}0.032$ (0.048) \\
\bottomrule
\end{tabular}
\end{table}

The dose interaction is null on both distress outcomes: interest coverage
($p=0.934$) and EBIT margin ($p=0.389$) show no evidence that the exposure
gradient steepens with tightening severity. It is significant only for the
implied rate itself ($p=0.048$)---which is the mechanical restatement, at firm
level, of the aggregate result already established in Table~\ref{tab:dose},
not a new finding about cross-sectional exposure. Scaling the test across a
range of monetary shocks that spans more than an order of magnitude produces
the same conclusion as the two-country design: the price of debt responds to
dose, the firm-level exposure gradient does not.
...
credit quality, and the four-year panel available for seven of these nine
countries is, by the feasibility condition of
Proposition~\ref{prop:feas}, too short to support the reliability-tested
repricing beta of Section~\ref{sec:exposure}. This subsection therefore
corroborates the two-country null under a cruder exposure measure and a wider
dose range; it does not repeat the decade-long identification test, which
remains specific to T\"{u}rkiye and Vietnam.

Two further limitations bear on Table~\ref{tab:dose} specifically. First,
Hungary and Mexico contribute only 23 and 19 firms respectively---fewer than
the 83-firm Turkish sample that Section~\ref{sec:robust} shows produced a
significant, wrongly signed coefficient before the sample expanded. Their
positions in Figure~\ref{fig:dose} should be read as suggestive rather than
precise, and the correlation of $r=0.983$ is not robust to their exclusion in
any strong sense we would claim: with only nine points, two of them thin,
the coefficient is better read qualitatively---coverage falls in every
country, and no country tightens without its borrowing costs moving---than
as a precisely estimated slope. Second, these seven countries have not been
